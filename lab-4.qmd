...
-   `echo` Shows unexecuted code chunk in render if true. When false, it is not rendered.
-   `warning` If code chunk cannot be executed, error message is allowed to be rendered.
-   `eval` Code chunk is executed when rendering if set to 'true'.
...
b.  You have the following code chunk, but look carefully, it's not exactly the same!
    ...
    When true, the 'error' code chunk renders any error associated with the chunk when set to 'true'.
...
All summary statistics appear similar and with little variance.
...
datasaurus_dozen |>
  ggplot(aes(x = x, y = y, color = dataset)) +
    geom_point(show.legend = FALSE) + facet_wrap(~dataset)

```

Visualizing a dataset can provide more information on trends and clustering not apparent when using summary statistics.
